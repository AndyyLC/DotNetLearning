...
public class XkcdClientTests
{
    ...
    public readonly HttpMessageHandler _fakeMsgHandler;
    ...
    """;
     ...
     //for raw string the {} cannot be on the same line as """

    public XkcdClientTests() 
    {
        _fakeMsgHandler = A.Fake<HttpMessageHandler>();
        var httpClient = SetupHttpClient (_fakeMsgHandler); //sets up HttpClient with handler
    ...
    internal static HttpClient SetupHttpClient( HttpMessageHandler msgHandler)
    {
        ...
        httpClient.BaseAddress = new Uri("https://xkcd.com"); //all URI are relative to this address 
        return httpClient;
    }
    ...
    private void SetResponse(HttpStatusCode statusCode, string content ="") 
    {
        ...
        A.CallTo(_fakeMsgHandler)
            .WithReturnType<Task<HttpResponseMessage>>() //send async is a ptrotected method
            .Where(c => c.Method.Name == "SendAsync") //matches call by name and return type
            .Returns(new HttpResponseMessage() //responds with status code and content
            {
