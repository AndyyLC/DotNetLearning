...
app.MapGet("/", (int? skip, int? take, FiggleTextDirection? dir, string? name, string? order) 
    => {var query = from f in AsciiArt.AllFonts.Value where (name == null || f.Name.Contains(name, SCIC)) && (dir == null || f.Font.Direction == dir)
    select f;
...
app.MapGet("/{text}", (string text, string? font) => AsciiArt.Write(text, out var asciiText, font) 
? Results.Content(asciiText!) : Results.NotFound());
...
app.MapGet("/barcode/{text}", (string text) => { 
    ...
    return Results.File(barcode.Encoded_Image_Bytes, "image.jpg");
});
