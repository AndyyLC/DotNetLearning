...
public static class AsciiArt
{
    ...
    public static Lazy<IEnumerable<(string Name, FiggleFont Font)>> AllFonts = 
        new (() => from p in typeof(FiggleFonts).GetProperties(BindingFlags.Public | BindingFlags.Static) 
    ...
    public static bool Write(string text, out string? asciiText, string? fontName = null) {
        ...
        {
            font = typeof(FiggleFonts).GetProperty(fontName, BindingFlags.Static | BindingFlags.Public)?.GetValue(null) as FiggleFont;
        }
        else
        {
